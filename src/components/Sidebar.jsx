...
export default function Sidebar({ user, active, onNavigate, onLogout }) {
        ...
        <div className="h-full bg-slate-900 text-white flex flex-col w-64">
            <div className="px-5 py-6 border-b border-slate-800">
                ...
                <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-sm font-bold">{initials}</div>
                    <div>
                        <div className="font-semibold text-sm">{user.name}</div>
                        <span className="inline-block mt-1 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide rounded bg-slate-700 text-slate-200">{user.role}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="p-3 border-t border-slate-800">
                <button onClick={onLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white">
    ...
    return (
        <>
            <button onClick={() => setMobileOpen(true)} className="md:hidden fixed top-4 left-4 z-40 bg-slate-900 text-white p-2 rounded-lg shadow-lg">
            ...
            {mobileOpen && (
                <div className="md:hidden fixed inset-0 z-50 flex">
                    ...
                    <div className="relative">
                        <button onClick={() => setMobileOpen(false)} className="absolute -right-10 top-3 text-white"><X /></button>
                        {content}
                    </div>
                </div>
            )}
